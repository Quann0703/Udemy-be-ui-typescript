...
function Topic() {
  ...
  useEffect(() => {
    if (Number(searchParams.get("page")) > 0 && !(show || isShowDelete)) {
      topicService
        .getTopics({ page: Number(searchParams.get("page")) })
        .then((res) => {
          setData(res.data);
        });
    }
  ...
  return (
    <div className={cx("wrapper")}>
      <Container className="my-4">
        <Row className="mb-3">
          <Col>
            <h3 className="fs-2 fw-bold">Topic</h3>
          </Col>
        </Row>
        <Row>
          <Col>
            <Card>
              <Card.Header className="d-flex justify-content-between align-items-center">
                <Card.Title className="mb-0">Topic Table</Card.Title>
